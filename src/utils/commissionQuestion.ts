...
export const commissionQuestion = (selectedOptionListProps: selectedOptionIndexObject[]) => {
  if (!selectedOptionListProps || selectedOptionListProps.length === 0) return [];
  ...
  // keyRenameMap을 활용해 매핑된 객체 배열 생성
  const transformed = selectedOptionListProps.map(obj => {
