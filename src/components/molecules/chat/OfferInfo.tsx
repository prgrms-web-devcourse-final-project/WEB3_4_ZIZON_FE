...
// 견적서 정보
export default function OfferInfo({ offerInfo, type, service }: OfferInfoProps) {
  return (
    <div className="w-full flex flex-col gap-16 px-32 py-32 rounded-[16px] bg-white border-1 border-priamry6 ">
      ...
      <div className="h-fit">
        <SelectedOption type="right-impact" leftText="서비스" rightText={service} />
        <SelectedOption
          type="right-impact"
          leftText="예상금액"
          rightText={`총 ${formatMoney(offerInfo.price.toString())} 원`}
        />
      </div>
      {type === 'client' && (
        <div className="flex flex-col gap-16">
          ...
          <Link
            href={`/expert/${offerInfo.expertId}`}
